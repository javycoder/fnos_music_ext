...
def install_platform_block():
    ...
    start = text.index("MDL_PLATFORM_TABLE='")
...
parse_sources "lxmusic-KuGou,mdl-48,mdl-GequhaiMusicClient"
...
printf "menu [%s]\\n" "$(menu_answer_to_tokens "1,6,9")"
printf "menu [%s]\\n" "$(menu_answer_to_tokens "1,2,8")"
printf "menu [%s]\\n" "$(menu_answer_to_tokens "lx-kw,musicdl-kuwo")"
'''
    ...
    # 裸 token（旧写法）= 整源默认平台，不动平台键
    assert rows[1] == 'bare 1 1 1 [] [] 0 0'
    # lx 别名 / musicdl 编号与全名输入归一
    assert rows[2] == 'alias [kg] [gequhai]'
    # 同源多平台并集去重
    assert rows[3] == 'union [kw,kg]'
    # musicdl-all = 显式默认平台，短名 → 全名映射
    assert rows[4] == 'all [kuwo,migu] [KuwoMusicClient,MiguMusicClient]'
    # 菜单编号 → token（1 网易云 / 6 lx-酷我 / 9 mdl-酷我；默认 1,2,8）
    assert rows[5] == 'menu [musicbox,lx-kw,musicdl-kuwo]'
    assert rows[6] == 'menu [musicbox,lx-all,musicdl-all]'
    assert rows[7] == 'menu [lx-kw,musicdl-kuwo]'
...
def test_parse_sources_rejects_unknown_platforms():
    ...
    result = run_install_bash(block + 'menu_answer_to_tokens "15"')
    assert result.returncode != 0 and '未知菜单编号' in result.stderr
