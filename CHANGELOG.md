...
自动迭代测试构建（未经完整人工验证，仅供尝鲜反馈；相关问题请到主仓库反馈，
以主仓库后续正式发布为准）。安装提速 + 歌单功能真机闭环验证。

### 修复

- **全新安装/无缓存构建从十几分钟降到约 2 分钟**：实测定位到根因——阿里云
  debian/pypi 镜像 CDN 对 HTTP/1.1 连接限速约 350KB/s（curl HTTP/2 走
  21MB/s 不受影响，而 apt 与 pip 只使用 HTTP/1.1）。apt/pip 默认源整体从
  阿里云切换到腾讯云镜像（apt 下载实测约 20MB/s），涉及 Dockerfile、
  docker-compose、install.sh、ensure_proxy_deps.sh、.env.example；存量
  .env 里写死的阿里云/清华历史默认值升级时自动迁移到腾讯云，用户自定义
  源地址一律保留不动。
- **搜索翻页官方条目跨页重复**：官方搜索接口越界页不返回空列表而是钳制回
  第 1 页（本地匹配只有 4 条时翻第 2 页仍返回同样 4 条；收藏/歌单条目接口
  无此行为）。本地优先合并布局下，官方回声会拼上在线切片导致同一批本地
  条目在每一页重复出现、单页条目数超出 size。本页全局起点越过官方段时
  现在会丢弃官方回声，翻页恢复不重不漏。

### 新增

- **e2e 冒烟脚本 `scripts/e2e_check.sh`**（开发完成/发版前必跑）：一键覆盖
  代理接管健康、搜索、分页、在线取流播放、收藏→列表可见、取消收藏→列表
  更新、播放上报→播放历史、建歌单→加在线歌→曲目列表/计数→移出→删歌单
  全链路，全部通过输出 ALL PASS。
- **fpk manifest 版本携带字母迭代修订号**（如 2.2.9d → `2.2.9-4`）：此前
  字母后缀被剥掉、所有迭代版 manifest 都是同一个 `2.2.9`，应用中心无法
  区分迭代。注意：当前 fnOS 的 CLI `install-fpk` 对已安装应用一律跳过、
  不做原地升级（提升基础版本号也一样），升级需先卸载再安装。
- 歌单在线条目闭环（2.2.9c）真机验证通过：命令行安装 141 秒完成（无
  Docker 缓存），e2e 13 项全绿。

## [2.2.9c] - 2026-09-23

自动迭代测试构建（未经完整人工验证，仅供尝鲜反馈；相关问题请到主仓库反馈，
以主仓库后续正式发布为准）。补齐歌单方向的在线曲目闭环：

### 修复

- **在线歌曲加不进自建歌单（官方库不认在线 id，静默失败）**：官方
  add-track/remove-track 对数据库中不存在的曲目 id 返回"参数无效"，在线曲目
  （客户端持有的是伪装成官方形态的假 id）直达官方必被拒。现在拦截这两个接口：
  在线条目记入本地存储（`playlist_tracks/`，按用户分文件、按歌单分桶，重复
  添加幂等并刷新加入时间，与官方语义一致），本地曲目照旧原样透传官方；同批
  混选本地+在线曲目时自动拆分各走各的，官方部分被拒则整体返回官方错误。
- **歌单里看不到已加入的在线歌曲 / 计数不含在线歌曲**：歌单曲目列表
  （track/playlist-detail/list）透传官方后合并下发本地在线条目（接在官方条目
  之后、按加入时间倒序，分页翻页不重不漏，红心状态按本地收藏实时显示）；
  playlist/detail 与 batch-detail 的 trackCount 同步并入在线条目数。从未加过
  在线歌曲进歌单的环境完全走原透传路径，零开销零风险。
- **删除歌单后残留在线附加数据**：官方删除成功后顺手清掉本地为该歌单存的
  在线条目。每日/热门推荐歌单保持只读（加歌/删歌请求直接吸收，不再报错）。
- 代理重启后，歌单内在线条目的伪装 id 反解映射可从 playlist_tracks 存储
  完整重建（与收藏/播放历史同款机制）。

## [2.2.9b] - 2026-09-23

自动迭代测试构建（未经完整人工验证，仅供尝鲜反馈；相关问题请到主仓库反馈，
以主仓库后续正式发布为准）。一批来自代码审查的修复：

### 修复

- **重新扫码登录后仍是旧登录态（VIP 曲目持续被过滤、推荐报未登录）**：登录由
  CLI 子进程写盘，而服务进程内的网易实例只在启动时读一次 cookie。现在扫码
  成功（code 803）立即丢弃常驻实例重建，登录态探测结果也加了有效期（成功
  5 分钟、失败 1 分钟），过期自动重读磁盘 cookie，不再需要切源/重启恢复。
- **网易在线点播偶发转圈后 404**：取流每次起 CLI 子进程（冷启动 1~3 秒）撞上
  代理侧 4 秒预算。取链改为服务进程内直接调用（毫秒级，失败自动降级 CLI），
  结果短缓存 10 分钟；代理单次解析预算放宽到 6 秒。
- **"最近播放"里删不掉在线歌曲**：列表是代理拼的、删除却转发给了官方。新增
  play-history/delete 拦截：在线条目删本地存储，官方条目原样转发。
- **边听边存对"分段拉流"的播放器完全不生效**：只有"从 0 拉到尾"的请求才落盘。
  现在这类播放器首次取流时，服务端后台整轨下载落盘（客户端请求不受影响），
  同时解决"听了一半缓存只有半首"。失败 30 分钟内不重试。
- **转发响应缺少总长度（Content-Length）**：本地歌曲经代理播放变成无总长的
  分块传输，个别播放内核时长显示异常或起播慢。现在 identity 响应原样透传
  精确长度，对齐官方直连行为。
- **推荐歌单 size=-1 只返回前 50 首**："返回全部"的约定值被 `<1` 兜底吃掉，
  现在正确返回全量。

### 新增

- **落盘进曲库后自动通知官方重扫**（默认关闭）：新增环境变量
  `FNMUSIC_LIBRARY_SCAN_PATH`，配置官方扫描接口路径后，边听边存落盘会带当前
  请求凭证触发扫描（3 秒合并窗口 + 90 秒节流）。官方扫描接口路径暂无公开
  资料：可在官方 App 手动点一次扫描、从代理日志（`journalctl -u
  fnmusic-ext.service`）捕获真实路径后填入启用。

## [2.2.9a] - 2026-09-23

自动迭代测试构建（未经完整人工验证，仅供尝鲜反馈；相关问题请到主仓库反馈，
以主仓库后续正式发布为准）。尝试修复两个用户反馈的问题：

### 修复

- **后台播放卡住：歌曲播到一部分（常见 70%-80%）停住，或播完不自动切下一首、
  进 App 一直转圈**：在线音源取流的读超时从 10 秒放宽到 60 秒，CDN 限速或
  短暂停顿不再掐断音频流；曲目结束时的落盘与音频标签写入移入后台线程，不再
  阻塞代理的其他请求（疑似"播完不切歌"的诱因）；音频流被中断时现在会记录到
  系统日志，`journalctl -u fnmusic-ext.service` 中查 `Stream aborted mid-way`
  即可取证反馈。
- **安装/升级失败："代理 unit 属于其他目录；拒绝停止或覆盖"**：报错信息现在
  带原目录与当前目录的具体路径和三种处理方式；原目录已被删除时自动接管，
  不再卡死；`--adopt` 迁移部署同时作用于 unit 与部署登记；应用升级时自动
  清理指向旧目录的失效 unit（迁卷后升级循环失败的根源）；卸载时还原失败会
  降级为强制清理，保证卸载总能完成。
...
- **安装时拉不到依赖：清华镜像源近期对部分网络返回 403**：pip 与 apt 的默认源
  从清华换成阿里云（`mirrors.aliyun.com`），安装与升级不再依赖单一镜像。
  宿主机 pip 的回退链同时调整为 阿里云 → 清华 → 官方 PyPI，一家不可用时自动换下
  一家重试。已装机用户升级时，配置里仍是历史默认清华源的会自动迁移到阿里云；
  手动改过源地址的不受影响。
...
- **搜索在音源回包前就交出空列表**：在线搜索只留一个超时，默认 15 秒。有在线结果立刻返回，本地歌曲在前；到时还没有就放弃这次还没回来的结果，只返回本地歌曲。管理页「音乐源」下方可改「搜索超时时间」，保存后立即生效。
- **搜索在停手 1 秒后才发给音源**：1 秒内继续输入就换成最新关键词并重新计时，窗口过去才真正搜索。
- **输入过程中搜索先空白、过一会儿再搜才有结果**：每个音源同时只搜一个关键词。同一用户继续输入时只保留最后那个词；洛雪会取消上一轮，musicdl 和网易等当前请求结束后丢掉旧结果再搜。被换掉的空结果不再缓存。
- **洛雪源 URL 测试显示 502**：桌面网关连接 WebUI 时把 5 秒建连超时留在了套接字上，校验要几十秒才返回，连接被中途拆掉，页面只看到 502。转发超时改为 180 秒。
- **电脑上传和 NAS 选择 .js 都失败**：管理页把脚本当 JSON `{filename, script}` 提交，接口却只接受 multipart，于是提示「请用 multipart/form-data 上传文件」。现两种格式都接受。

### 新增

- **搜索结果本地优先**：接管后官方搜索把本地曲库与在线音源结果按页交错混排，
  在线条目还会挡住后续页的本地结果。现改为全局统一布局——本地条目占满前
  `本地总数` 位、在线条目（网易 > musicdl > 洛雪）紧随其后，翻页顺序严格
  「先本地、后在线」，`total` = 本地 + 在线。
- **洛雪源支持文件方式配置（重要）**：管理页「音乐源 → 洛雪自定义源」新增三种
  配置方式——粘贴 `http(s)` URL（原有）、上传电脑上的 `.js` 文件（multipart →
  容器 `sources-data/lxmusic/uploads/` 落盘，持久化随数据卷）、从 NAS 选择
  `.js`（飞牛桌面内经开放 API `pickUserFile` 文件选择器选中，由宿主侧网关代读
  内容后同样落盘）。上传脚本以 `file:///data/lxmusic/uploads/<名字>.js` 形态
  参与统一的「测试 → 保存 → 激活」链路；manifest 声明 `micro_app=true` 并申请
  `trim.file.userAccess` scope。
- **`install.sh --lx-source-url` 支持本机路径**：传入宿主机上存在的 `.js` 文件
  路径时自动复制进 `sources-data/lxmusic/uploads/` 并转为 `file://` 挂载地址，
  校验/激活链路复用不变。

### 变更

- **安装向导不再索要洛雪源 URL**：fpk 向导移除 `wizard_lx_url` 输入框与
  `wizard_lx_skip_verify` 开关（平台向导无文件上传组件），选 lxmusic 直接完成
  安装（无源状态），源脚本装好后在管理页配置；CLI `--lx-source-url` 变为可选。
  升级流程兼容 `.env` 中 `LX_SOURCE_URL` 为空的部署。

## [2.2.6] - 2026-09-22

### 修复

- **飞牛桌面里的管理窗口一片空白（重要）**：桌面页面是 HTTPS，管理入口却把 `http://主机:8774` 嵌进 iframe，浏览器丢弃混合内容，窗口只有标题栏。改为走桌面网关的同源路径 `/app/fnmusic-ext/`，由本机 socket 转到 WebUI。直连 8774 仍然可用。
- **应用中心安装在启用扩展时静默失败（重要）**：`extend.sh` 用 `read` 重读 `VERSION`。该文件没有末尾换行时 `read` 返回 1，`set -e` 在打出任何日志前退出。安装脚本已经印完成功提示，弹窗却只剩一份即将被回滚删掉的日志路径。改为 `head` 读取。
- **安装/升级日志在回滚后仍保留**：过程输出追加到 `/var/log/apps/fnmusic-ext-install.log`（写不了则落到 `/tmp/fnmusic-ext-install.log`）。失败弹窗带上该路径；没有 `[ERROR]` 行时附上日志末尾。
- **musicdl 搜索连续几次都是 0 首歌**：一次搜索向每个平台要 60 首（6 页），平台还要逐首确认直链。等不及就记一次失败，下次等待被压到 3 秒，短到一页都回不来，界面就是空的；上一轮还占着平台时，新的搜索会立刻被拒绝。现改为每个平台只取一页（10 首），超时下限放到 8 秒，已经交出歌曲就不再把等待往下压。空结果只缓存 10 秒。

### 文档

- README 标明 Gitee / GitHub 仓库地址，并说明 musicdl 部分平台歌曲少或不可播放、洛雪源能否搜索和播放取决于所填 URL，需自行选用可靠源。
