...
- **搜索首屏确定性空结果加固（Issue #18 后续）**：
  - 搜索首屏空结果时自动对失败音源做一次有界重试，缓解瞬时故障导致的误报「无结果」。
  - 仅对硬失败（返回 None）的音源协程进行重试重建，成功源结果保留不重复请求，受总 deadline 严格约束。

## [2.2.7a] - 2026-09-22

### 优化

- **搜索响应延迟优化与首屏超时容错（针对 Issue #18）**：
  - 调整在线搜索首屏等待时间预算 netease_wait_s 默认由 3.0s 优化为 1.8s，未命中结果时的最大兜底等待 late_page_wait_s 默认由 5.0s 优化为 3.0s。
  - 解决移动端（如 iOS 飞牛 App）首次键入搜索时因多源聚合等待过长导致客户端网络超时或列表为空的问题，兼顾首屏响应速度与聚合成功率。
